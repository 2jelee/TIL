...
public class MybatisController {
	...
	servlet-context.xml에서 생성한.
	 */
	@Autowired
	private SqlSession sqlSession;
//	public void setSqlSession(SqlSession sqlSession) {
//		this.sqlSession = sqlSession;
//		System.out.println("Mybatis 사용준비 끝"); // 주입되었는지 확인하기 위해 setter생성.
//	}
...
//		parameterDTO.setSearchTxt(req.getParameter("searchTxt")); //[2차 버전]
...
//		System.out.println("totalRecordCount="+totalRecordCount);
		...
		//기존(리스트 1)과는 다르게 시작, 끝을 DTO에 저장함
...
//		추상메소드를 호출한다. 
//		그러면 mapper에 정의된 쿼리문이 실행되는 형식으로 동작함.
//		 */
...
//	}
	...
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		/*
		매핑된 메소드 내에서 session 내장객체를 사용하기 위해서
		매개변수로 선언해준다.
		 */
		...
		if(session.getAttribute("siteUserInfo")==null) {
			/*
			글쓰기 페이지 진입 시 세션이 없는 상태로
			로그인 페이지 이동 시 로그인 후에는 다시 글쓰기 페이지로
			돌아가기 위해 backUrl을 지정해 준다.
			 */
	...
	public ModelAndView loginAction(HttpServletRequest req, HttpSession session) {
		...
		if(vo==null) {
			//로그인에 실패한 경우 실패메세지를 model에 저장한 이후,
			mv.addObject("LoginNG", "아이디/패스워드가 틀렸습니다.");
			//다시 로그인 페이지를 호출한다.
			mv.setViewName("07Mybatis/login");
			return mv;
		}
		else {	//로그인에 성공한 경우 세션영역에 속성을 저장한다. (VO객체 자체를 통째로 저장)
			session.setAttribute("siteUserInfo", vo);
		}
		
		// 로그인 후 돌아갈 페이지가 없는 경우에는 로그인 페이지로 이동. (backUrl)
	...
	public String writeAction(Model model, HttpServletRequest req, HttpSession session) {
		...
		//글쓰기 처리를 위한 메소드 호출
		//Mybatis 사용
//		int result = sqlSession.getMapper(MybatisDAOImpl.class).write( //write에서 int타입
		sqlSession.getMapper(MybatisDAOImpl.class).write(
			req.getParameter("name"), 
			req.getParameter("contents"),
			((MemberVO)session.getAttribute("siteUserInfo")).getId()
		); //세션 영역에 저장된 VO객체로부터 아이디를 얻어와서 파라미터로 사용
//		System.out.println();
	...
	public String modifyAction(HttpSession session, MyBoardDTO myBoardDTO) {
		//수정 페이지에서 전송된 폼값은 커맨드객체를 통해 한꺼번에 받음
...
//			model.addAttribute("backUrl", "07Mybatis/modify");
	...
	public String delete(HttpServletRequest req, HttpSession session) {
		...
		/*
		삭제처리 위해 delete() 메소드 호출. 
		삭제처리의 경우에도 삭제된 레코드 갯수가 정수형으로 반환된다.
		필요없는 경우 생략할 수 O
		*/
