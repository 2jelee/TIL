...
public class MVCBoardDAO extends ConnectionPool {
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			rs = psmt.executeQuery();
			rs.next();
			if(rs.getInt(1)==0) { //패스워드가 일치하는 게시물이 없으므로 false
				isCorr = false;
			}
		}
		catch (Exception e) { //예외가 발생하여 확인이 불가능하므로 false
			isCorr = false;
			e.printStackTrace();
		}
		return isCorr; //return result;?
	}
...
//	게시판 글 삭제하기
//	Model - model2.mvcboard.MVCBoardDAO.java 에 메소드 추가
	//삭제 처리
	public int deletePost(String idx) {
		int result = 0;
		try {
			//비밀번호 검증 후 즉시 삭제하므로 비밀번호는 조건에서 제외함.
...
//
//	Model
//	model2.mvcboard.MVCBoardDAO.java 에 메소드 추가
