...
요청이 들어왔을때 Auto Scan된다.
	<< 이는 클래스 위 문자(S)여부로 확인할 수 있음
 */
@Controller
public class BbsController {
	...
	/*
	Service 객체를 해당 컨트롤러에서 사용하기 위해 스프링 컨테이너가
	미리 생성해둔 빈을 자동주입 받는다.
	이를 위해 servlet-context.xml에 component-scand을 하나 추가한다.
	그리고 해당 패키지 하위에서 자동으로 생성하고 싶은
	클래스에 @Service 또는 @Repository 어노테이션을 추가하면 된다.
	 */
	@Autowired
...
//		command = new ListCommand();		<< new 없애기
		command = listCommand;
...
//		JDBCTemplateDAO dao = new JDBCTemplateDAO(); //상단의 private
		...
		if(rowExist<=0) {
			...
			if(mode.equals("edit")) {
				//수정이면 수정폼으로 이동한다.
				model.addAttribute("req", req);
//				command = new EditCommand(); //상단의 @Autowired로 변환
				command = editCommand;
				command.execute(model);
