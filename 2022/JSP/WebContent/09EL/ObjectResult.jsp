...
<body>
	...
	<%
	/* 
	JSP코드를 통해 영역에 저장된 속성을 가져올 때에는
	1. getAttribute()으로 속성을 읽어온다.
	2. 형변환 한다
	...
	*/
	...
	<ul>
		<li>
			MemberDTO객체 : <%=String.format("아이디:%s,"
					...
					member.getId(),
					member.getPass(),
					member.getName()) %> <!-- rerererereererererererre -->
		</li>
		<!-- String, Integer와 같은 기본객체는 별도의 형변환 없이 사용할 수 O-->
		<li>
	...
	산술연산을 위해서는 반드시 int형으로 변환이 필요하다.
	*/
	...
	<!--
	영역에 저장된 속성명이 중복되지 X라면,
	xxxScope 내장객체를 사용하지 X하고도 읽어올 수 있고
	형변환도 별도로 하지 않아도 된다.
	또한 산술연산 시 int형으로 변환도 필요 X.
	getter의 호출없이 멤버변수의 이름만으로 저장된 값을 바로 읽어 출력할 수 있다.
	따라서 JSP코드를 사용할 때 보다 EL을 사용하면 훨씬 더 간편해진다.
	 -->
	...
	<ul>
		<!-- 200과 300의 합의 결과가 500이 출력됨  -->
		<li> ${param.firstNum + param['secondNum'] }</li> <!-- 결과 : 500   << EL식 내에서 연산-->
		
		<!-- 단순한 출력이므로 200+300으로 출력됨 -->
		<li> ${param.firstNum} + ${param["secondNum""] }</li> <!-- 결과 : 200+300   << 변수를 각각 출력이므로-->
	</ul>
</body>
</html>