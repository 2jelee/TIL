...
한글이 포함되어 있다면 '반드시' 포워드 시키는 '최초 페이지'에서 인코딩을 설정해야 한다!!
*/
...
<body>
	...
	(아래와 같이 시작과 종료태그가 있는 경우에는 사이에 HTML주석(<!-- )을 사용할 수 없다!
	error 발생됨.)  
	  -->
	...
	/* 
	request 영역에 저장된 객체는 forward된 페이지까지 영역이 공유되므로
	영역을 통해 데이터를 전달할 수 있다.
	차후 Servlet(서블릿)에서 활용되는 방식이다.
	*/
