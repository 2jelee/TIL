...
XML(eXtensive Markup Language)
 ...
 - HTML과 비슷하게 태그로 표현하지만
 	XML은 개발자가 직접 태그를 정의할 수 O
 - HTML과 같이 보여주는 목적이 아닌,
 	데이터를 '전달'하는 것이 주목적이다.
 	주로 서버간 통신에 사용됨.
 전혀 사용X는 아니나, JSON을 많이 사용. (XML은 JSON에 비해 사용도 낮)
 
 *파싱(parsing)
 - 주어진 데이터를 분석하여 원하는 데이터의 특정 패턴이나 순서로
   추출해서 가능하는 것을 말한다.
 - 즉, 쉽게말해 XML이나 HTML등으로 구성된 데이터를 분석해
   내가 원하는 부분만 출력하는 것을 말함.
 -->
 	<!-- XML데이터를 set태그를 통해 변수로 저장함.  -->
	<c:set var="booklist">
		<booklist>
			<book>
				<name>사피엔스</name>
	...
	<!-- 
	해당 데이터(XML)를 파싱할때 사용.(파싱하겠다는 의미)
	XML데이터를 blist로 지정함. 
	-->
	...
	<h4>파싱1</h4>
	<!--  
	출력하고자 하는 XML의 노드에 접근하여 해당 엘리먼트를 출력.
	XML의 노드에 접근할 때 아래와 같이 '경로형태'로 접근하는 것을 XPath라고 한다.
	
	book[1] => 첫번째 엘리먼트를 표시함. 배열처럼 0부터 시작하지 X.
	-->
	제목 : <x:out select="$blist/booklist/book[1]/name"/> <br /> <!-- XPath는 1부터 시작함 (0부터 시작X) -->
	...
	<table border="1">
		...
		<tr>
			...
			<td>
				<!--
					choose를 통해 조건식을 사용함.
					core와는 다르게 select 속성을 사용. (core >> text속성 사용했음)
				-->
				<x:choose>
