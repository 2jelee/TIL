...
public class MainActivity extends AppCompatActivity {
    ...
    /*
    AsyncTask
        : 작업쓰레드와 관련된 복잡한 작업을 쉽게 처리해주는 클래스이다.
    ...
    class CounterTask extends AsyncTask<Integer, Integer, Integer> {
        //doInBackground() 메소드 실행전에 호출되는 메소드
        ...
        protected Integer doInBackground(Integer ... value) {
            while (mProgressStatus < 100) {
                try {
                    //0.1초에 한번씩 block상태로 들어간다.
                    Thread.sleep(100);
                }
                catch (InterruptedException e) {
                }
                mProgressStatus++;
                //onProgressUpdate() 을 호출하는 함수로 진행상황에 대한 표현을 한다.
        ...
        /*
        doInBackground()가 실행된 후 결과값을 해당 메소드로 전송한다.
        즉 정상적으로 종료되었을때 호출된다.
         */
