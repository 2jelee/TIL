...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fragmentTransaction.replace(R.id.mainLayout, menuFragment2).commit();
    }
        ...
        public void onClick(View view) {
            if(view.getId()==R.id.btnFirstFragment){
                fragmentManager.beginTransaction()
                        .replace(R.id.mainLayout, menuFragment1).commit();
            }
            else if(view.getId()==R.id.btnSecondFragment){
                fragmentManager.beginTransaction()
                        .replace(R.id.mainLayout, menuFragment2).commit();
            }
            else if(view.getId()==R.id.btnThirdFragment){
                fragmentManager.beginTransaction()
                        .replace(R.id.mainLayout, menuFragment3).commit();
            }
            else if(view.getId()==R.id.btnFourthFragment){
                fragmentManager.beginTransaction()
                        .replace(R.id.mainLayout, menuFragment4).commit();
            }
        }
    };
}