...
<body>
	<!-- 
	set태그의 속성									//rerereerere elVar : null <- 0이 나와야한다.
		var : 변수명으로 문자열만 가능
		value : 값을 설정.
				표현식, EL, 일반값 모두 가능함.
	...
	<ul>
		...
		<li>
		2. value속성에 EL로 값 대입 :
			<c:set var="expVar" value="${directVar mod 5}" />
		</li>
		<li>
	...
	<!--
	EL로 영역의 속성을 읽어올 때에는 xxScope를 생략하면
	...
	<!--
	영역에 저장될 때 Object형으로 자동 캐스팅되므로
	사용 시 해당 자료형으로 형변환해야 한다.
	...
	<!-- 
	EL로 출력 시에는 별도의 형변환이 필요 X,
	속성명의 중복이 없다면 내장객체를 생략할 수 있으므로 편리함.
	 -->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!--
	1. 변수설정.
		new를 통해 객체를 생성한 후 setAtrribute()으로 영역에 저장
	 -->
	<c:set var="defaultMember" value="<%=new MemberDTO() %>"
	scope="request" /> <!-- 빈(객체)을 생성하는 부분  -->
	
	<!-- 
	2. 생성된 변수의 멤버변수값을 지정.
		setter를 통한 값 설정
	-->
	<!-- target을 이용(연결)하여 빈에 값을 설정하는 부분 -->
	...
	index로 접근되므로 객체에 접근시 인덱스를 사용함.
	 -->
	<h2>List 컬렉션 이용하기</h2> <!-- 배열과 같음. 순서 보장. index를 통한 접근 O -->
	<%
	...
	<!-- 
	0번 인덱스에 저장된 defaultMember객체를 지정하여 정보를 출력함.
	정지훈에 대한 정보가 출력됨
	 -->
	...
	key를 통한 접근이 index를 통한 접근보다 속도가 빠름!
	 -->
	<h2>Map 컬렉션 이용하기</h2>
	<%
	Map map = new HashMap();
