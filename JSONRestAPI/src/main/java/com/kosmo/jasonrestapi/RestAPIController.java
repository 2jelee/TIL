...
public class RestAPIController {
	...
	/*
	Mybatis를 사용하기 위해 빈을 주입받는다.
	 */
	@Autowired
	private SqlSession sqlSession;
	
	//모델 1방식의 게시판인 board테이블의 목록을 JSON으로 얻어옴.
	@RequestMapping("/restapi/boardList.do")
	@ResponseBody 		//반환타입이 List컬렉션이므로 JSON배열로 출력함.
	public ArrayList<BoardDTO> boardlist(HttpServletRequest req) {
		...
		/*
		검색어를 스페이스로 구분하여 2개이상 입력하면 
		split()메소드를 통해 분리하여 List컬렉션에 저장한다.
		 */
