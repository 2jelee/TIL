...
//jQuery의 진입점
$(function() {
	//목록 불러오기 버튼을 클릭했을 때 ajax()를 호출한다.
	$('#btnBoard').click(function(){
		$.ajax({
			...
			success : sucCallBack, //성공 시 콜백메소드
			error : errCallBack //실패 시의 콜백메소드
		});
	});
	//페이지가 로드되면 trigger()함수를 통해 click이벤트를 발생시킴.
	//즉, 우리가 직접 버튼을 클릭한 것과 동일한 액션을 실행함.
...
function sucCallBack(resData) {
	...
	let tableData = "";
	//JSON배열이므로 each() 함수를 통해 갯수만큼 반복함.  **each << JSON에서 중요한
	...
	//해당 위치에 있던 새로운 내용으로 대체함.
	$('#show_data').html(tableData);
	//$('#show_data').append(tableData); <<append로 변경해본다면? 페이지가 늘어나는.
}
//실패했을 때의 콜백 메소드
