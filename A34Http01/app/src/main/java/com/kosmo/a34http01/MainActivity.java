...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnJson.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                ...
                /*
                 ...
                 첫번째 인수로 접속URL을 전달한다. execute() 메소드 호출을 통해
                 doInBackground()가 호출된다.
                 */
                new AsyncHttpRequest().execute(
                        "http://192.168.219.104:8081/jsonrestapi/android/memberList.do"
                );
            }
        });
        ...
        /*
        스타일, 아이콘, 타이틀, 메세지, back버튼을 눌렀을때의 설정 등을 하고있음
         */
    ...
    {
        //doInBackground() 메소드 실행전에 호출됨
        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            //서버와 통신 직전 진행대화창이 있는지 확인후 띄워준다.
            if(!dialog.isShowing())
                dialog.show();
        }

        /*
        excute() 호출시 전달된 파라미터를 가변인자가 받게된다.
        가변인자는 여러개의 파라미터를 하나의 변수로 받을수 있도록
        배열로 사용하게 된다.
         */
        @Override
        protected String doInBackground(String... strings) {
            //스프링 API 서버에서 반환하는 JSON데이터를 저장할 변수
            StringBuffer sBuffer = new StringBuffer();
            try{
                //0번째 인자를 통해 전달된 서버URL을 인자로 URL객체를 생성
                ...
                /*
                서버로 요청할 파라미터를 설정한다. 해당 예제에서는 통신시 전달할
                별도의 파라미터가 없으므로 아래 부분에 별다른 설정은 없다.
                 */
                ...
                if(buttonResId == R.id.btn_json){
                    ...
                    //StringBuffer 객체를 비워줌
                    sBuffer.setLength(0);
                    for(int i=0 ; i<jsonArray.length() ; i++){
                        //배열의 크기만큼 반복하면서 JSON객체를 키값을 통해 파싱한다.
            ...
            /*
            파싱이 완료된 StringBuffer객체를 String으로 변환후 반환한다.
