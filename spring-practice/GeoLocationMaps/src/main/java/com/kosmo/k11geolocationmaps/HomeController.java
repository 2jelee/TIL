...
public class HomeController {
	...
	private static final String apiKey = "AIzaSyCzl0XxDKZWJXOvXJSv7TvaScvPeLUqyuc";
	...
	public String geoFunc4(Model model, HttpServletRequest req) {
		...
		model.addAttribute("apiKey", apiKey);
		//폼값받기 (Stirng으로 전송된 값을 정수 혹은 실수형으로 변경함)
		...
		int resultCount = sqlSession.getMapper(ISearchRadius.class).searchCount(distance, latTxt, lngTxt);
		model.addAttribute("resultCount", " / 검색결과:"+resultCount+"건");
		//전체 페이지 수 (검색결과 / 출력할 갯수)
		/*
		전체 페이지 수 (검색결과 / 출력할갯수) - 페이지 번호를 대체할 <select> 태그에서 사용할 속성값
		 */
		...
		//위에서 계산한 값과 파라미터를 로그로 출력
		System.out.println(distance + " " + latTxt + " " + lngTxt + " "+ start + " " + end);
		
		//검색결과 가져오기
		ArrayList<MyFacilityDTO> searchList = null;
		if(distance!=0) {
			searchList = sqlSession.getMapper(ISearchRadius.class)
