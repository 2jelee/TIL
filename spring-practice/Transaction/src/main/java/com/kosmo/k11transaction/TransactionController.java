...
//		/*
//		클라이언트가 전송한 폼값을 커맨드 객체를 통해 한번에 받아 DAO로 전달된다.
//		 */
...
//	} 
	...
	**********************/
	...
	2. 트랜잭션 템플릿을 활용한 처리
	*/
	//자동주입 시 setter를 사용하지 않아도 됨. 멤버변수에 직접 적용할 수 있음.
	
	@Autowired
	private TicketTplDAO daoTpl;
//	public void setDatoTpl(TicketTplDAO daoTpl) {
//		this.daoTpl = daoTpl;
//	}
	//구매 페이지
	...
	public String buyTicketAction(TicketDTO ticketDTO, Model model) {
		...
		if(isBool==true) {
			//정상처리된 경우 모든 작업은 commit 된다.
			model.addAttribute("successOrFail", "티켓구매가 정상처리 되었습니다.");
		}
		else {
			//오류가 발생한 경우에는 모든 작업이 rollback 된다.
