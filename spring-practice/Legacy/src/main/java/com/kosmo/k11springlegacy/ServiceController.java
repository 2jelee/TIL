...
/*
servlet-context.xml에서 component-scan으로 설정된 기본패키지에 
컨트롤러 클래스 역할을 부여하기 위한 어노테이션.
스프링 컨테이너 시작 시 자동으로 스캔되어 빈이 생성된다.
 */
@Controller
public class ServiceController {
	
//	@Autowired 		<< 아래대신 위에 붙여도 상관 없. 또한 아래 setter를 없어도 된다.(현재는 흔적을 남기기 위해 만듦)
	/*
						@Autowired
						MyService myService;
	*/
	/*
	서비스 객체를 주입받는다.
	@Autowired는 멤버변수, 생성자, setter 메소드에 사용할 수 있다.
	*/
	MyService myService;
	@Autowired
	...
	일반적으로 setter메소드 없이 멤버변수에 직접 어노테이션을 부착한다.
	 */
	@Autowired
	private LottoService lottoService;
	
	/*
	사용자가 HTML페이지에서 링크를 클릭할 때 전달하는 userLottoNum 파라미터를
