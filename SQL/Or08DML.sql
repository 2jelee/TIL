...
설명 : 레코드를 조작할 때 사용하는 쿼리문. 앞에서 학습했던 select 문을 비롯하여
        update(레코드 수정), delete (레코드 삭제), insert (레코드입력)가 있다.
*************************************/
...
레코드 입력하기 : insert문
    ...
    숫자형은 ''없이 그냥 쓴다. 만약 숫자형을 ''로 감싸면 자동으로 형변환되어 입력됨!
*/
...
--where 절 업이 모든 레코드를 대상으로 지역을 '가산디지털'로 변경하시오.
