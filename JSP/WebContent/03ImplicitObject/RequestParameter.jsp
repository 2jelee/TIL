...
<body>
<%
//한글깨짐 방지											///코드 rerererererere
request.setCharacterEncoding("UTF-8");

/*
전송방식에 상관없이 getParmeter()로 폼값을 받을 수 있다.
...
if(!(name==null || name.equals("")) ){
	...
	//스크립트릿 내부에 JS를 작성할때는 out.println()을 사용함
	/* out.println("<script>");
...
%>
	<!-- 
	위와 같은 방법은 불편하므로 스크립트릿을 분리한 후
	HTML영역을 만들어 JS를 기술한다.
	-->
	<script>
	alert('이름을 입력하세요');
	history.back();
...
/*
radio는 항목이 여러개지만 선택가능한 항목이 1개이기 때문에
getParameter()를 통해 폼값을 받을 수 있다.
*/
String sexStr = request.getParameter("sex");

/*
checkbox는 2개 이상의 항목을 선택할 수 있으므로 getParameterValues()를 사용함.
항목 중 체크된 것만 서버로 전송되고
파라미터명이 틀리거나 값이 전송되지 않으면 null을 반환함.
*/
...
출력 시에는 특수기호를 <br />로 변환 후 출력해야 한다.
*/
...
<%
/* 
getParameterNames() : 폼값의 이름을 한꺼번에 받아올 때 사용하는 메소드로,
	반환타입은 Enumeration형이다.
	
	ㄴ하위 함수
	hasMoreElements() : 객체에 남은 폼값의 이름이 있는지 확인함.
	nextElement() : 받은 폼값 중 다음 폼값의 이름을 반환한다.
*/
Enumeration names = request.getParameterNames();
while(names.hasMoreElements()){

	//폼의 name속성을 하나씩 읽어서 String형으로 반환하다.
