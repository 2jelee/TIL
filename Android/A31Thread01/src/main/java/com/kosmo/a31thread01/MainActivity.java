...
public class MainActivity extends AppCompatActivity {
    ...
    public void onBtn1Clicked(View v){
        ...
        호출해야 쓰레드가 생성된다. 만약 run()을 호출하면 단순한 실행만될뿐
        쓰레드는 생성되지 않는다.
         */
        RequestThread thread = new RequestThread();
        thread.start();//쓰레드 시작작
   }
    ...
    class RequestThread extends Thread {
        ...
        public void run() {
            for(int i=0 ; i<20 ; i++){
                ...
                //textView1.setText("Request Thread .."+ i);// 예외 발생됨
                ...
                ANR이란?
                    ...
                    의미를 쉽게 파악할 수 있다. '애플리케이션이 응답하지 않는다.' 인
                    것이다. 이 에러의 원인은 Main Thread(UI Thread)가 일정 시간
    ...
    /*
    핸들러 클래스 생성
        : 메인쓰레드에서 생성한 UI(위젯)는 다른 쓰레드에서 접근할 수
    ...
    class ProgressHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            쓰레드의 동작이 끝나기전에는 버튼을 누를수 없도록 비활성화
            상태를 유지한다.
             */
            if (data2.equals("19")) {
