...
public class TicketDAO {
	...
	: Spring-JDBC를 사용하기 위한 멤버변수와 setter() 정의
      트랜젝션 처리를 위한 매니저 클래스의 멤버변수와 setter() 정의
	 */
	JdbcTemplate template;
	PlatformTransactionManager transactionManager;

	//setter (초기화 시켜주기 위해)
	//servlet-context.xml에서 빈을 생성할 때 setter()를 사용해서 초기화 함.
		...
		try {
			...
			template.update(new PreparedStatementCreator() {
				...
				public PreparedStatement createPreparedStatement(Connection con) 
					...
					psmt.setString(1, dto.getCustomerId()); //티켓 1장에 10,000원이라고 가정
			...
			/*
			티켓 구매장수가 5장 잉하인 경우 정상처리 되어 모든 작업 commit된다.
			 */
			template.update(new PreparedStatementCreator() {
			...
			/*
			티켓 구매장수가 5장을 초과하는 경우에는 예외가 발생하여 모든 작업이
			취소되고, rollback된다.
			 */
			System.out.println("제약조건을 위배하여 카드결제와 티켓구매 모두가 취소"
					+ "되었습니다.");
